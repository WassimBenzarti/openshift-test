...
function ImageLoader(data,container){
  for(var i=0;i<data.length;i++){
    ...
    var likeBtn = $('<span class="likeBtn" title="like">').load('/inc/logo/icons/like1.svg').attr('data-likes',d.likes).data('id',d.id).addClass(cookieLiked(d.id)).appendTo(img);
    var commentBtn = $('<span class="commentBtn" title="comment">').load('/inc/logo/icons/comment.svg').attr('data-comments',d.comments).click(function(){
